...
class Endicia
{
    ...
    function __construct($RequesterID, $AccountID, $PassPhrase)
    {
           ...
           if($this->debug == false) {
               $this->client = new Client('https://LabelServer.Endicia.com');
           }else {
               $this->client = new Client('https://LabelServer.endicia.com');
...
// Note you may want to associate this in some other way, like with a database ID.
    ...
    $this->PartnerTransactionID = substr(uniqid(rand(), true), 0, 10);
        ...
        $data = array("changePassPhraseRequestXML" => $xml);
        $request = $this->client->post('/LabelService/EwsLabelService.asmx/ChangePassPhraseXML', array(), $data);
        $this->send_request($request);
